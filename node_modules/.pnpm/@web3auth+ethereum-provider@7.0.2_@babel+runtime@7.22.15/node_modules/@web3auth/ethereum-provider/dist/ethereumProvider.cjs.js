...
;// CONCATENATED MODULE: external "@walletconnect/utils"
...
;// CONCATENATED MODULE: ./src/providers/injectedProviders/walletConnectV2Utils.ts
...
async function sendJrpcRequest(signClient, chainId, method, params) {
  ...
  if (!session) {
    throw rpc_errors_namespaceObject.providerErrors.disconnected();
  }
...
;// CONCATENATED MODULE: ./src/providers/injectedProviders/WalletConnectV2Provider.ts
...
class WalletConnectV2Provider extends base_provider_namespaceObject.BaseProvider {
  ...
  async enable() {
    if (!this.connector) throw rpc_errors_namespaceObject.providerErrors.custom({
  ...
  checkIfChainIdAllowed(chainId) {
    ...
    const allChains = (0,utils_namespaceObject.getChainsFromNamespaces)(sessionData.namespaces);
    let chainAllowed = false;
    for (const chain of allChains) {
      const parsedId = (0,utils_namespaceObject.parseChainId)(chain);
    ...
    connector.events.on("session_event", async payload => {
      ...
      if (event.name === "chainChanged") {
        ...
        if (connectedHexChainId && this.state.chainId !== connectedHexChainId) {
          const maybeConfig = (0,base_namespaceObject.getChainConfig)(base_namespaceObject.CHAIN_NAMESPACES.EIP155, connectedHexChainId) || {};
...
_class = WalletConnectV2Provider;
...
function ethPrivatekeyUtils_getProviderHandlers(_ref) {
  ...
  return {
    ...
    processDecryptMessage: (msgParams, _) => {
      ...
      const stripped = (0,util_namespaceObject.stripHexPrefix)(msgParams.data);
...
var EthereumPrivateKeyProvider_class;
...
EthereumPrivateKeyProvider_class = EthereumPrivateKeyProvider;
defineProperty_default()(EthereumPrivateKeyProvider, "getProviderInstance", async params => {
  const providerFactory = new EthereumPrivateKeyProvider_class({
    config: {
      chainConfig: params.chainConfig
    }
  });
